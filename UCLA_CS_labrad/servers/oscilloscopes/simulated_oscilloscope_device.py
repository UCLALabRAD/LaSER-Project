...
class SimulatedKeysightDSOXADevice(GPIBDeviceModel):
    ...
    def measure_frequency(self,chan):
        chan=int(chan[-1])
        waveform=self.channels[chan-1].generate_waveform()
        wavelength_starts=find_where_crossing(self,waveform)
    ...
    def autoscale(self):
        ...
        for channel in self.channels:
            if channel.input_signal_log:
                for signal in channel.input_signal_log.log:
                    if signal.func:
                        channel.is_on=True
                        break
                    channel.is_on=False
        ...
        while True:
            ...
            waveform=channel.generate_waveform(self.max_window_horizontal_scal)
