...
class HardwareSimulationServer(LabradServer):
    ...
    def add_device(self, c, node, address,instr_model,is_gpib):
        ...
        if issubclass(self.sim_instr_models[instr_model].cls,SimulatedGPIBInstrumentInterface):
            self.wrapped_simulated_devices[(node,address)]=SimulatedGPIBCommunicationInterface(self.sim_instr_models[instr_model].cls())
        else:
            self.wrapped_simulated_devices[(node,address)]=SimulatedSerialCommunicationInterface(self.sim_instr_models[instr_model].cls())
    ...
    def remove_device(self,c, node, address):
        ...
        for context_obj in self.contexts.values():
            if context_obj.data['Device'] is self.wrapped_simulated_devices[(node,address)]:
                ...
                break
        del self.wrapped_simulated_devices[(node,address)]
    ...
    def select_device(self,c,node,address):
        if c['Device']:
            raise HSSError(4)
        c['Device']=self.wrapped_simulated_devices[(node,address)]
